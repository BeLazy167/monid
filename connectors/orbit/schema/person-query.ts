 ...
 * only, no `.default()`. Orbit's own defaults are applied at each BINDING, so
 * every estimate reads concrete numbers.
 *
 * Orbit requires at least one of `query`, `intent` or `signals` on a search
 * and on every bulk item. That is a cross-field rule, and cross-field rules
 * do not survive JSON Schema compilation — it is stated in the descriptions
 * that carry these fragments, and Orbit answers a request satisfying none of
 ...
 *  each body so both compile to one flat object. */
export const personQueryShape = {
