...
/**
 * The person-query fragments — how a caller DESCRIBES the people they want.
 * Shared by `orbit#v3/search` and `orbit#v3/search/bulk`, whose item shape is
 * the single-search shape plus a caller-assigned `id`.
 *
 * Faithful mirrors of the published v3 components (design D25): optionality
 * only, no `.default()`. Orbit's own defaults are applied at the BINDING in
 * each endpoint.ts, so an estimate reads a concrete number rather than
 * guessing at one.
 *
 * Orbit requires at least one of `query`, `intent` or `signals` on a search.
 * That is a cross-field rule, and cross-field rules do not survive JSON
 * Schema compilation — it is stated in every description that carries these
 * fragments, and Orbit answers a request that satisfies none of them with a
 * `400`, which arrives as data.
 */
...
/** The fields a single search and one bulk item hold in common. Spread into
 *  each body rather than extended, so each endpoint's compiled schema reads
 *  as one flat object. */
export const personSearchShape = {
