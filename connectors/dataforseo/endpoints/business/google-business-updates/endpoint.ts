...
import { zGoogleBusinessUpdatesBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Business Posts — `POST
 * /v3/business_data/google/my_business_updates/task_post` (v1
 * `/google-business/updates`). Page-billed: $0.0045 per page of 10 results;
...
export default defineEndpoint({
    meta: {
        displayName: "Google Business Posts",
        summary:
            "Fetch the posts (updates) a business published on its Google " +
            "profile.",
        description:
            "Updates posted on a business's Google profile, found by name " +
            "and location. Returns per post the text, date, images, and " +
            "links. Supports depth (10 per page). Suited for monitoring " +
            "competitor promotions and profile activity. To find the " +
        ...
        docsUrl:
            "https://docs.dataforseo.com/v3/business_data/google/my_business_updates/task_post/",
        categories: ["maps"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 10 updates.",
        ],
    },
    endpoint: "/google-business/updates",
    request: {
        method: "POST",
    ...
    input: {
        schema: {
            body: zGoogleBusinessUpdatesBody.extend({
                depth: zGoogleBusinessUpdatesBody.shape.depth.unwrap().default(
                    10,
                ),
            }),
        },
    ...
    usage: {
        model: {
            ...
            unit: Unit.RESULT,
            every: 10,
            consumes: { credit: "default", amount: 0.0045 },
