...
import { zGoogleBusinessExtendedReviewsBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Extended Reviews — `POST
 * /v3/business_data/google/extended_reviews/task_post` (v1
 * `/google-business/extended-reviews`). Page-billed: $0.0015 per page of 20
 * results; the hold and the count are the results asked for, the vendor's
 * default when omitted (design D4 / D5). Queued upstream: task_post at high
 * priority, task_get polled (design D6).
 */
export default defineEndpoint({
    meta: {
        displayName: "Google Extended Reviews",
        summary: "Fetch Google reviews plus reviews from linked third-party " +
            "sites.",
        description:
            "Google reviews of a business by cid or place_id (or name) " +
            "plus reviews Google aggregates from other sites such as " +
            "Booking or Tripadvisor. Returns per review the source, " +
            "author, rating, text, and date. Supports depth (20 per page, " +
            "up to 1000). Suited for a fuller reputation picture than " +
            "Google-only reviews. To find the location_code or exact " +
            "location_name for a city or country, call " +
            "dataforseo#google-business/locations (free lookup, country " +
            "filter + search).",
        docsUrl:
            "https://docs.dataforseo.com/v3/business_data/google/extended_reviews/task_post/",
        categories: ["maps"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 20 reviews; searching by keyword instead " +
            "of cid or place_id adds $0.003.",
        ],
    },
    endpoint: "/google-business/extended-reviews",
    request: {
        method: "POST",
    ...
    input: {
        schema: {
            body: zGoogleBusinessExtendedReviewsBody.extend({
                depth: zGoogleBusinessExtendedReviewsBody.shape.depth.unwrap()
                    .default(20),
            }),
        },
    },
