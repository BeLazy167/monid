...
import { zTripadvisorSearchBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Tripadvisor Search — `POST /v3/business_data/tripadvisor/search/task_post`
 * (v1 `/tripadvisor/search`). Page-billed: $0.0015 per page of 30 results;
...
export default defineEndpoint({
    meta: {
        displayName: "Tripadvisor Search",
        summary: "Search Tripadvisor for places and get their ratings and " +
            "review counts.",
        description: "Tripadvisor search results for a keyword and location. " +
            "Returns places with rank, url_path, title, category, rating, " +
            "review count, and address. Supports depth (30 per page). " +
            "Suited for finding a venue's Tripadvisor page before pulling " +
            "reviews. To find the location_code or exact location_name " +
            "for Tripadvisor, call dataforseo#tripadvisor/locations (free " +
            "lookup, country filter + search).",
        docsUrl:
            "https://docs.dataforseo.com/v3/business_data/tripadvisor/search/task_post/",
        categories: ["hotels"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 30 listings.",
        ],
    },
    endpoint: "/tripadvisor/search",
    request: {
        method: "POST",
    ...
    input: {
        schema: {
            body: zTripadvisorSearchBody.extend({
                depth: zTripadvisorSearchBody.shape.depth.unwrap().default(30),
            }),
        },
    },
