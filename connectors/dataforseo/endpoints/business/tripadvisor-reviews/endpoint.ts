...
/**
 * Tripadvisor Reviews — `POST
 * /v3/business_data/tripadvisor/reviews/task_post` (v1
 * `/tripadvisor/reviews`). Page-billed: $0.0015 per page of 10 results; the
...
export default defineEndpoint({
    meta: {
        ...
        summary: "Fetch Tripadvisor reviews of a place by its URL path.",
        description: "Tripadvisor reviews for a place by url_path (from " +
            "tripadvisor-search) or name. Returns the place's rating and " +
            "review count, then per review the author, rating, title, " +
            "text, date, trip type, and owner response. Supports depth " +
            "(10 per page, up to 4490), rating filter, and sort_by. " +
            "Suited for hospitality reputation monitoring. To find the " +
            "location_code or exact location_name for Tripadvisor, call " +
            "dataforseo#tripadvisor/locations (free lookup, country " +
            "filter + search).",
        docsUrl:
            "https://docs.dataforseo.com/v3/business_data/tripadvisor/reviews/task_post/",
        categories: ["hotels"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 10 reviews; setting a language adds " +
            "$0.0015.",
        ],
    },
    ...
    input: {
        schema: {
            body: zTripadvisorReviewsBody.extend({
                depth: zTripadvisorReviewsBody.shape.depth.unwrap().default(10),
            }),
        },
    },
