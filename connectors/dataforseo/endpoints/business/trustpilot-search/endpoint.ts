...
import { zTrustpilotSearchBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Trustpilot Search — `POST /v3/business_data/trustpilot/search/task_post`
 * (v1 `/trustpilot/search`). Page-billed: $0.0015 per page of 10 results;
...
export default defineEndpoint({
    meta: {
        displayName: "Trustpilot Search",
        summary: "Search Trustpilot for businesses and get their ratings and " +
            "review counts.",
        description:
            "Trustpilot search results for a keyword. Returns businesses " +
            "with rank, domain, title, rating, review count, categories, " +
            "and location. Supports depth (10 per page). Suited for " +
            "finding a company's Trustpilot profile before pulling " +
            "reviews.",
        docsUrl:
            "https://docs.dataforseo.com/v3/business_data/trustpilot/search/task_post/",
        categories: ["company-reviews"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 10 listings.",
        ],
    },
    endpoint: "/trustpilot/search",
    request: {
        method: "POST",
    ...
    input: {
        schema: {
            body: zTrustpilotSearchBody.extend({
                depth: zTrustpilotSearchBody.shape.depth.unwrap().default(10),
            }),
        },
    },
