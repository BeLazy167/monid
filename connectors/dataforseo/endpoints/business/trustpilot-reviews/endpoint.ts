...
import { zTrustpilotReviewsBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Trustpilot Reviews — `POST /v3/business_data/trustpilot/reviews/task_post`
 * (v1 `/trustpilot/reviews`). Page-billed: $0.0015 per page of 20 results;
...
export default defineEndpoint({
    meta: {
        ...
        summary: "Fetch Trustpilot reviews of a company by domain.",
        description:
            "Trustpilot reviews for a company domain. Returns the profile " +
            "rating and review count, then per review the author, rating, " +
            "title, text, date, verified flag, and company reply. " +
            "Supports depth (20 per page) and sort_by. Suited for " +
            "reputation monitoring and review mining.",
        docsUrl:
            "https://docs.dataforseo.com/v3/business_data/trustpilot/reviews/task_post/",
        categories: ["company-reviews"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 20 reviews.",
        ],
    },
    endpoint: "/trustpilot/reviews",
    request: {
        method: "POST",
    ...
    input: {
        schema: {
            body: zTrustpilotReviewsBody.extend({
                depth: zTrustpilotReviewsBody.shape.depth.unwrap().default(20),
            }),
        },
    },
