...
import { zGooglePlayAppReviewsBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Play App Reviews — `POST /v3/app_data/google/app_reviews/task_post`
 * (v1 `/google-play/app-reviews`). Page-billed: $0.0015 per page of 150
...
export default defineEndpoint({
    meta: {
        displayName: "Google Play App Reviews",
        summary: "Fetch user reviews of a Google Play app with ratings and " +
            "replies.",
        description:
            "Reviews of a Google Play app by app_id. Returns per review " +
            "the author, rating, text, date, helpful votes, app version, " +
            "and the developer reply. Supports depth (150 per page), " +
            "rating filter, and sort_by (newest, most relevant). Suited " +
            "for sentiment mining and feature-request discovery. To find " +
            "the location_code or exact location_name for a Google Play " +
            "storefront, call dataforseo#google-play/locations (free " +
            "lookup, country filter + search).",
        docsUrl:
            "https://docs.dataforseo.com/v3/app_data/google/app_reviews/task_post/",
        categories: ["app-stores"],
        notes: [
            "Queued at high priority (about a minute).",
            "Billed per page of 150 reviews.",
        ],
    },
    endpoint: "/google-play/app-reviews",
    request: {
        method: "POST",
    ...
    input: {
        schema: {
            body: zGooglePlayAppReviewsBody.extend({
                depth: zGooglePlayAppReviewsBody.shape.depth.unwrap().default(
                    150,
                ),
            }),
        },
