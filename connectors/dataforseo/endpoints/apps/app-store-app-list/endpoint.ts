...
import { zAppStoreAppListBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * App Store Top Charts — `POST /v3/app_data/apple/app_list/task_post` (v1
 * `/app-store/app-list`). Page-billed: $0.0024 per page of 100 results; the
...
export default defineEndpoint({
    meta: {
        displayName: "App Store Top Charts",
        summary:
            "Fetch an App Store chart: top free, top paid, or grossing by " +
            "category.",
        description:
            "App Store chart for an app_collection (top free, top paid, " +
            "top grossing) and optional app_category. Returns apps with " +
            "rank, app_id, title, developer, rating, and price. Supports " +
            "depth (up to 1000, 100 per page). Suited for category " +
            "leaderboard tracking. To find category ids for the App " +
            "Store, call dataforseo#app-store/categories (free lookup).",
        docsUrl:
            "https://docs.dataforseo.com/v3/app_data/apple/app_list/task_post/",
    ...
    endpoint: "/app-store/app-list",
    request: { method: "POST", path: "/v3/app_data/apple/app_list/task_post" },
    input: {
        schema: {
            body: zAppStoreAppListBody.extend({
                depth: zAppStoreAppListBody.shape.depth.unwrap().default(100),
            }),
        },
    },
