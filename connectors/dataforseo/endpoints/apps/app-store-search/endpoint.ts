...
import { zAppStoreSearchBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * App Store Search — `POST /v3/app_data/apple/app_searches/task_post` (v1
 * `/app-store/search`). Page-billed: $0.0024 per page of 100 results; the
...
export default defineEndpoint({
    meta: {
        displayName: "App Store Search",
        summary: "Search the App Store and get ranked apps for a keyword.",
        description: "App Store search results for a keyword and storefront " +
            "location. Returns apps with rank, app_id, title, developer, " +
            "rating and votes, price, icon, and description snippet. " +
            "Supports depth (up to 700, 100 per page) and language. " +
            "Suited for ASO rank tracking and app discovery.",
        docsUrl:
            "https://docs.dataforseo.com/v3/app_data/apple/app_searches/task_post/",
    ...
    input: {
        schema: {
            body: zAppStoreSearchBody.extend({
                depth: zAppStoreSearchBody.shape.depth.unwrap().default(100),
            }),
        },
    },
