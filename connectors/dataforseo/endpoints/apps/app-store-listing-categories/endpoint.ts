...
/**
 * App Store Listing Categories — `GET
 * /v3/app_data/apple/app_listings/categories` (v1
 * `/app-store/listing-categories`). Free dictionary lookup (design D7).
 */
export default defineEndpoint({
    meta: {
        displayName: "App Store Listing Categories",
        summary: "Look up the category ids the App Store app database search " +
            "accepts.",
        description: "Free lookup. Returns the category ids and names for " +
            "app-store-listings. Search matches the name. To use these " +
            "values in /app-store/listings, call " +
            "dataforseo#app-store/listings (the endpoint this list is " +
            "for).",
        docsUrl:
            "https://docs.dataforseo.com/v3/app_data/apple/app_listings/categories/",
        categories: ["app-stores"],
    },
    endpoint: "/app-store/listing-categories",
    request: {
        method: "GET",
