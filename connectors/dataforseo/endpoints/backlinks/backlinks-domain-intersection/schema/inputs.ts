...
/**
 * Request body of `POST /v3/backlinks/domain_intersection/live` — the vendor's fields as
 * documented (docs.dataforseo.com, OpenAPI 89d7d681 2026-09-20); only
 * optionality, no defaults. Callback and queue fields (`postback_url`,
 * `pingback_url`, `postback_data`, `tag`, `priority`) are deliberately
 * absent — the object is strict, so they are rejected before any spend.
 */
export const zBacklinksDomainIntersectionBody = z.object({
    targets: z.record(z.string(), z.string().min(1)).describe(
        "Numbered targets, e.g. {'1': 'example.com', '2': 'other.com'}; up to 20.",
    ),
    exclude_targets: z.array(z.string().min(1)).describe(
