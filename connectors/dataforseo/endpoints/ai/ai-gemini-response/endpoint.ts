...
import { zAiGeminiResponseBody } from "./schema/inputs.ts";

/**
 * Ask Gemini — `POST /v3/ai_optimization/gemini/llm_responses/live` (v1
 * `/ai/gemini-response`). Flat: $0.0006 per call (design D4).
 */
export default defineEndpoint({
    meta: {
        displayName: "Ask Gemini",
        summary: "Send a prompt to Gemini and get the answer with citations " +
            "and token usage.",
        description:
            "Runs a prompt against a Gemini model through DataForSEO. " +
            ...
            "call dataforseo#ai/gemini-models (free lookup).",
        docsUrl:
    ...
    endpoint: "/ai/gemini-response",
    request: {
        method: "POST",
        path: "/v3/ai_optimization/gemini/llm_responses/live",
    },
    input: {
        schema: { body: zAiGeminiResponseBody },
