...
/**
 * Gemini Models — `GET /v3/ai_optimization/gemini/llm_responses/models` (v1
 * `/ai/gemini-models`). Free dictionary lookup (design D7).
 */
export default defineEndpoint({
    meta: {
        displayName: "Gemini Models",
        summary: "List the Gemini model names gemini-response accepts.",
        description:
            "Free lookup. Returns rows of model_name with each model's " +
            "web_search support. Search matches the name. To use these " +
            "values in /ai/gemini-response, call " +
            "dataforseo#ai/gemini-response (the endpoint this list is " +
            "for).",
        docsUrl:
            "https://docs.dataforseo.com/v3/ai_optimization/gemini/llm_responses/models/",
        categories: ["geo"],
    },
    endpoint: "/ai/gemini-models",
    request: {
        method: "GET",
