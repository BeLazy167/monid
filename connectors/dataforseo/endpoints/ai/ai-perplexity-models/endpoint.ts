...
/**
 * Perplexity Models — `GET
 * /v3/ai_optimization/perplexity/llm_responses/models` (v1
 * `/ai/perplexity-models`). Free dictionary lookup (design D7).
 */
export default defineEndpoint({
    meta: {
        displayName: "Perplexity Models",
        summary: "List the Perplexity model names perplexity-response accepts.",
        description:
            "Free lookup. Returns rows of model_name with each model's " +
            "web_search support. Search matches the name. To use these " +
            "values in /ai/perplexity-response, call " +
            "dataforseo#ai/perplexity-response (the endpoint this list is " +
            "for).",
        docsUrl:
            "https://docs.dataforseo.com/v3/ai_optimization/perplexity/llm_responses/models/",
        categories: ["geo"],
    },
    endpoint: "/ai/perplexity-models",
    request: {
        method: "GET",
