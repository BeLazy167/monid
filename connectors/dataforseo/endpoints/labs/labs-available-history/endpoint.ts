...
/**
 * Labs History Availability — `GET
 * /v3/dataforseo_labs/google/available_history` (v1
 * `/labs/available-history`). Free dictionary lookup (design D7).
 */
export default defineEndpoint({
    meta: {
        displayName: "Labs History Availability",
        summary: "See from which date historical Labs data exists per " +
            "location.",
        description:
            "Free lookup. Returns rows of location_code, location_name, " +
            "and the first available date of historical SERP and keyword " +
            "data. Search matches the location name. To use these values " +
            "in /labs/historical-serps, call " +
            "dataforseo#labs/historical-serps (the endpoint this list is " +
            "for). To use these values in /labs/historical-rank-overview, " +
            "call dataforseo#labs/historical-rank-overview (the endpoint " +
            "this list is for). To use these values in " +
            "/labs/historical-keyword-data, call " +
            "dataforseo#labs/historical-keyword-data (the endpoint this " +
            "list is for).",
        docsUrl:
            "https://docs.dataforseo.com/v3/dataforseo_labs/google/available_history/",
        categories: ["seo"],
    },
    endpoint: "/labs/available-history",
    request: {
        method: "GET",
