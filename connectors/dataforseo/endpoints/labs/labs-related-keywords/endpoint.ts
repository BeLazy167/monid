import { defineEndpoint, Unit, UsageModelKind } from "@shared/core";
import { zLabsRelatedKeywordsBody } from "./schema/inputs.ts";

/**
 * Related Keywords — `POST /v3/dataforseo_labs/google/related_keywords/live`
 * (v1 `/labs/related-keywords`). Per-row: $0.012 per request plus $0.00012
 * per row returned (design D4).
 */
export default defineEndpoint({
    meta: {
        displayName: "Related Keywords",
        summary: "Get keywords related to a seed via Google's 'searches " +
            "related to' graph.",
        description:
            "Related keywords for a seed keyword, walked up to three " +
            "levels deep through Google's related searches. Returns per " +
            "keyword the search volume, CPC, competition, difficulty, " +
            "intent, and SERP info, with the depth at which it was found. " +
            "Supports depth, filters, sorting, and up to 1000 rows. " +
            "Suited for topic clustering and content ideation. To see " +
            "which fields filters and order_by accept here, call " +
            "dataforseo#labs/filters (free lookup of filterable fields " +
            "per Labs endpoint). To find the location_code and " +
            "language_code pairs Labs supports, call " +
            "dataforseo#labs/locations (free lookup, search by country " +
            "name).",
        docsUrl:
            "https://docs.dataforseo.com/v3/dataforseo_labs/google/related_keywords/live/",
        categories: ["seo"],
    },
    endpoint: "/labs/related-keywords",
    request: {
        method: "POST",
        path: "/v3/dataforseo_labs/google/related_keywords/live",
    },
    input: {
        schema: {
            body: zLabsRelatedKeywordsBody.extend({
                limit: zLabsRelatedKeywordsBody.shape.limit.unwrap().default(
                    100,
                ),
            }),
        },
    },
    usage: {
        model: {
            kind: UsageModelKind.COMPOSITE,
            components: {
                base_fee: {
                    kind: UsageModelKind.PER_CALL,
                    consumes: { credit: "default", amount: 0.012 },
                    label: "base fee",
                    description: "the per-request fee",
                },
                rows: {
                    kind: UsageModelKind.PER_UNIT,
                    unit: Unit.RESULT,
                    consumes: { credit: "default", amount: 0.00012 },
                    label: "rows",
                    description: "items returned (result[0].items, or its " +
                        "items_count when the items were not returned)",
                },
            },
        },
        estimate: ({ data }) => ({ counts: { rows: data.input.body.limit } }),
    },
});
