...
import { zGoogleShoppingSellersBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Shopping Sellers — `POST /v3/merchant/google/sellers/task_post` (v1
 * `/google-shopping/sellers`). Page-billed: $0.002 per page of 20 results;
...
export default defineEndpoint({
    meta: {
        displayName: "Google Shopping Sellers",
        summary: "List sellers of a Google Shopping product with prices and " +
            "shipping.",
        description:
            "Sellers of a Google Shopping product by product_id (from " +
            "google-shopping-products). Returns per seller the name, " +
            "price, shipping and tax, total price, rating, and the store " +
            "link; ad links can be resolved with " +
            "google-shopping-seller-ad-url. Supports depth and sort_by. " +
            "Suited for price comparison across merchants. To find the " +
            "location_code or exact location_name for Google Shopping, " +
            "call dataforseo#google-shopping/locations (free lookup, " +
            "country filter + search). To resolve a seller's ad link to " +
            "the store URL, call dataforseo#google-shopping/seller-ad-url " +
            "(near-free lookup).",
        docsUrl:
    ...
    endpoint: "/google-shopping/sellers",
    request: { method: "POST", path: "/v3/merchant/google/sellers/task_post" },
    input: {
        schema: {
            body: zGoogleShoppingSellersBody.extend({
                depth: zGoogleShoppingSellersBody.shape.depth.unwrap().default(
                    10,
                ),
            }),
        },
