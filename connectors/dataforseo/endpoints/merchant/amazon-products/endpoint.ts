 ...
 * (v1 `/amazon/products`). Page-billed: $0.0033 per page of 48 results; the
...
export default defineEndpoint({
    meta: {
        ...
        description: "Amazon search results for a keyword in a marketplace " +
            ...
            "700, 48 per page), department, search_param, price range, " +
        ...
        categories: ["amazon"],
        notes: [
            "Billed per page of 48 results; each further page adds the " +
            "same price.",
        ],
    },
    ...
    input: {
        schema: {
            body: zAmazonProductsBody.extend({
                depth: zAmazonProductsBody.shape.depth.unwrap().default(48),
            }),
        },
    },
    usage: {
        model: {
            ...
            description:
                ...
                "billed per page of 48",
        },
        estimate: ({ data }) => ({
            counts: {
                RESULT: Math.max(
                    data.input.body.depth,
                    (data.input.body.max_crawl_pages ?? 1) * 48,
                ),
            },
        }),
    },
