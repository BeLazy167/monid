...
import { zGoogleShoppingProductsBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Shopping Search — `POST /v3/merchant/google/products/task_post` (v1
 * `/google-shopping/products`). Page-billed: $0.002 per page of 40 results;
...
export default defineEndpoint({
    meta: {
        displayName: "Google Shopping Search",
        summary: "Search Google Shopping and get ranked products with prices " +
            "and sellers.",
        description:
            "Google Shopping results for a keyword and location. Returns " +
            "products with rank, product_id, title, price and currency, " +
            "seller, rating and reviews, delivery info, and image, plus " +
            "filters and sponsored listings. Supports depth (up to 120), " +
            "price range, and sort_by. Suited for price comparison and " +
            "merchant monitoring. To find the location_code or exact " +
            "location_name for Google Shopping, call " +
            "dataforseo#google-shopping/locations (free lookup, country " +
            "filter + search).",
        docsUrl:
        ...
        notes: [
            ...
            "Billed per page of 40 results.",
        ],
    },
    endpoint: "/google-shopping/products",
    request: { method: "POST", path: "/v3/merchant/google/products/task_post" },
    input: {
        schema: {
            body: zGoogleShoppingProductsBody.extend({
                depth: zGoogleShoppingProductsBody.shape.depth.unwrap().default(
                    40,
                ),
            }),
        },
    ...
    usage: {
        model: {
            ...
            description:
                ...
                "billed per page of 40",
        },
        estimate: ({ data }) => ({
            counts: {
                RESULT: Math.max(
                    data.input.body.depth,
                    (data.input.body.max_crawl_pages ?? 1) * 40,
                ),
            },
        }),
    },
