...
import { zSerpGoogleAdsSearchBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Ads Library Search — `POST /v3/serp/google/ads_search/task_post`
 * (v1 `/serp/google-ads-search`). Page-billed: $0.0012 per page of 40
...
export default defineEndpoint({
    meta: {
        displayName: "Google Ads Library Search",
        summary: "List ads an advertiser ran in Google Ads Transparency " +
            "Center.",
        description:
            "Ads shown in Google's Ads Transparency Center for one or " +
            "more advertiser_ids (from google-ads-advertisers), " +
            "optionally filtered by date range, format (text, image, " +
            "video), and platform. Returns ads with creative id, format, " +
            "first and last shown dates, preview URL, and target regions. " +
            "Supports depth (40 per page). Suited for competitor creative " +
            "monitoring. To find the location_code or exact location_name " +
        ...
        notes: [
            ...
            "40 results; each further page adds the same price.",
        ],
    },
    endpoint: "/serp/google-ads-search",
    request: { method: "POST", path: "/v3/serp/google/ads_search/task_post" },
    input: {
        schema: {
            body: zSerpGoogleAdsSearchBody.extend({
                depth: zSerpGoogleAdsSearchBody.shape.depth.unwrap().default(
                    40,
                ),
            }),
        },
