...
import { zSerpGoogleSearchByImageBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Reverse Image Search — `POST
 * /v3/serp/google/search_by_image/task_post` (v1
 * `/serp/google-search-by-image`). Page-billed: $0.0012 per page of 100
...
export default defineEndpoint({
    meta: {
        displayName: "Google Reverse Image Search",
        summary: "Find pages and visually similar images for an image URL.",
        description:
            "Google search-by-image for an image_url. Returns pages that " +
            "contain the image or close matches with rank, title, URL, " +
            "domain, and snippet, plus visually similar images and the " +
            "best-guess label. Supports location and language. Suited for " +
            "image provenance checks and brand-asset misuse detection. To " +
        ...
        notes: [
            ...
            "100 results; each further page adds the same price.",
        ],
    },
    endpoint: "/serp/google-search-by-image",
    request: {
        method: "POST",
        path: "/v3/serp/google/search_by_image/task_post",
    },
    input: { schema: { body: zSerpGoogleSearchByImageBody } },
    ...
    usage: {
        ...
        estimate: ({ data }) => ({
            counts: { RESULT: (data.input.body.max_crawl_pages ?? 1) * 100 },
        }),
    },
});
