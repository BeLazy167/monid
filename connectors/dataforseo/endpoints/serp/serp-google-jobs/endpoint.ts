...
import { zSerpGoogleJobsBody } from "./schema/inputs.ts";
import { zTaskState } from "../../../schema/common.ts";

/**
 * Google Jobs Results — `POST /v3/serp/google/jobs/task_post` (v1
 * `/serp/google-jobs`). Page-billed: $0.0012 per page of 10 results; the
...
export default defineEndpoint({
    meta: {
        displayName: "Google Jobs Results",
        summary: "Search Google Jobs and get job postings with employer and " +
            "location.",
        description:
            "Google Jobs results for a keyword and location. Returns " +
    ...
    endpoint: "/serp/google-jobs",
    request: { method: "POST", path: "/v3/serp/google/jobs/task_post" },
    input: {
        schema: {
            body: zSerpGoogleJobsBody.extend({
                depth: zSerpGoogleJobsBody.shape.depth.unwrap().default(10),
            }),
        },
    },
