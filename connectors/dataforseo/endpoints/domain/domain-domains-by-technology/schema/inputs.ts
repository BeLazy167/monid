...
export const zDomainDomainsByTechnologyBody = z.object({
    technology_paths: z.array(z.string().min(1)).describe(
        "Target technology paths",
    ).optional(),
    groups: z.array(z.string().min(1)).describe(
    ...
    keywords: z.array(z.string().min(1)).describe(
        "Target keywords in the domain's title, description or meta keywords (required unless groups, technology_paths, technologies or categories optional field is given)",
    ).optional(),
