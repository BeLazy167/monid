...
Deno.test(`${ID} happy: free, envelope passes through`, async () => {
    ...
    const circuit = output.data.circuit as Record<string, unknown>;
    assertEquals(circuit.qubits, 2);
    assertEquals((circuit.gates as unknown[]).length, 4);
});
