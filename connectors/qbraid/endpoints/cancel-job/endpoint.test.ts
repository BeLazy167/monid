...
Deno.test(`${ID} happy: free, envelope passes through`, async () => {
    const result = await runEndpoint({
        ...
        input: INPUT,
        mode: "replay",
        fixture: await loadFixture(`${fixturesDir}synthetic-happy.json`),
    });
    ...
    assertEquals(output.data.status, "CANCELLING");
});
...
Deno.test(`${ID}: a job still initializing — 409 is data`, async () => {
    const result = await runEndpoint({
        unit: await testSealedUnit(ID),
        input: INPUT,
        mode: "replay",
        fixture: await loadFixture(`${fixturesDir}conflict.json`),
    });
